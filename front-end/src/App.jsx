...
export default function App() {
  ...
  const selectGroup = (group, passcode = null, callback = null) => {
    if (!group?.id) return;
    socket.emit('join-group', { groupId: group.id, passcode }, (result) => {
      if (callback) callback(result || { ok: false, error: 'No response' });
      ...
      if (!result.ok) {
        if (!callback) setError(result.error || 'Unable to join room.');
        return;
      }
      setActive(result.group || group);
      if (Array.isArray(result.userGroups)) {
  ...
  const authenticate = (details, callback) => {
    ...
    socket.emit('auth', details, (result) => {
      ...
      if (result?.ok && result.user) {
        ...
        setGroups(groupList);
        const initial = groupList.find((g) => !g.isPrivate || g.isMember || g.owner === result.user) || groupList[0];
  ...
  const handleInviteUser = (groupId, targetUsername) => {
    socket.emit('invite-user', { groupId, targetUsername }, (result) => {
      if (!result) return setError('Failed to send invite.');
      if (!result.ok) return setError(result.error || 'Failed to send invite.');
    ...
    <main className="chat-shell">
      ...
      {joiningGroup && (
        <PasscodeModal
          ...
          onSubmit={(room, passcode, callback) => selectGroup(room, passcode, callback)}
        />
      )}

      {showInviteModal && active && (
        <InviteModal
          room={active}
